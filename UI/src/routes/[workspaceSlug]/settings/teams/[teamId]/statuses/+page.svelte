<script lang="ts">
	...
	import { Plus, Trash2, Lock, Pencil, X, Check } from 'lucide-svelte';
		...
		{:else}
			<div class="rounded-lg border border-[var(--app-border)] overflow-hidden">
				{#each CATEGORY_ORDER as cat, catIdx}
					...
					{#each catStatuses as status}
						<div class="flex items-center gap-3 px-4 py-2.5 border-t border-[var(--app-border)] hover:bg-[var(--color-bg-hover)]/50 group transition-colors">
