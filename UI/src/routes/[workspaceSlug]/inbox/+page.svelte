<script lang="ts">
	...
	import {
		Inbox, Clock, Archive, Eye, AlarmClock, ExternalLink,
		ArrowRightLeft, UserCheck, MessageSquare, AtSign,
	...
	function getTypeStyle(type: string) {
		return NOTIFICATION_TYPE_STYLE[type] ?? { icon: Inbox, color: '#6b7280', bg: 'rgba(107,114,128,0.12)' };
	}
		...
		<div class="flex w-[320px] shrink-0 flex-col border-r border-[var(--app-border)]">
			...
			<div class="flex-1 overflow-y-auto">
				...
				{:else if notifications.length === 0}
					<div class="px-4 py-8">
						<EmptyState
							title={activeTab === 'inbox' ? 'No notifications' : activeTab === 'snoozed' ? 'No snoozed' : 'No archived'}
					...
					{#each notifications as notification (notification.id)}
						...
						>
							...
							<div class="flex-1 min-w-0">
								<p class="text-[12px] leading-snug text-[var(--color-text-primary)] line-clamp-2">{notification.title}</p>
								...
								<div class="flex shrink-0 items-center gap-0.5 opacity-0 group-hover:opacity-100">
									<Popover.Root open={snoozeOpenId === notification.id} onOpenChange={(open) => { snoozeOpenId = open ? notification.id : null; }}>
										...
										<Popover.Content class="w-32 p-1" align="end">
											...
											<button onclick={() => { snoozeOpenId = null; handleSnooze(notification.id, 3); }} class="flex w-full rounded-md px-2 py-1 text-xs text-[var(--color-text-primary)] hover:bg-[var(--color-bg-hover)]">3 hours</button>
											<button onclick={() => { snoozeOpenId = null; handleSnooze(notification.id, 24); }} class="flex w-full rounded-md px-2 py-1 text-xs text-[var(--color-text-primary)] hover:bg-[var(--color-bg-hover)]">Tomorrow</button>
										</Popover.Content>
									</Popover.Root>
			...
			{:else if selectedIssue}
				{#key selectedIssue.id}
					<FullPageIssueView
						issue={selectedIssue}
						{slug}
						onupdated={(updated) => { selectedIssue = updated; }}
					/>
				{/key}
