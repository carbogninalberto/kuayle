...
func (r *TeamRepository) Update(ctx context.Context, team *domain.Team) error {
	query := `UPDATE teams SET name = $1, description = $2, color = $3, icon = $4, estimate_scale = $5, triage_enabled = $6, updated_at = NOW() WHERE id = $7 RETURNING updated_at`
	return r.db.QueryRowContext(ctx, query, team.Name, team.Description, team.Color, team.Icon, team.EstimateScale, team.TriageEnabled, team.ID).Scan(&team.UpdatedAt)
}
